...
function PREPARE_PROD {
	...
	if [[ $(ssh centos@proddb sudo -i -u postgres whoami) ]]; then
		psql -d dafdb -h proddb -p 5432 -U postgres -c 'ALTER TABLE USERS DROP CONSTRAINT username;'
	elif [[ $(ssh centos@proddb sudo -i -u oracle whoami) ]]; then
...
{
	until [[ $(pgrep startup.sh) || $(pgrep -f ansible-playbook) ]] ; do
		echo "Waiting for startup.sh to start"
		sleep 3
	done
	while [[ $(pgrep startup.sh) || $(pgrep -f ansible-playbook) ]] ; do
